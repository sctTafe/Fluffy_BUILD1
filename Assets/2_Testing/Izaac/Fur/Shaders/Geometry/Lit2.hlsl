...
// Updated Attributes structure with a new furMask field.
// Note: We have reassigned TEXCOORD semantics so that furMask is passed along.
struct Attributes
{
    float4 positionOS : POSITION;
    float3 normalOS : NORMAL;
    float2 texcoord : TEXCOORD0;
    float2 lightmapUV : TEXCOORD1;
    float furMask : TEXCOORD2; // New: sampled in the vertex shader
    uint id : TEXCOORD3;
};
...
{
    // Update fur mover data as before.
    ...
    // Build the Attributes output and sample the fur mask.
    Attributes output;
    output.positionOS = input.positionOS;
    output.normalOS = input.normalOS;
    output.texcoord = input.texcoord;
    output.lightmapUV = input.lightmapUV;
    // Sample the mask here in the vertex shader.
    // (Using the green channel, adjust if necessary.)
    float furMask = (_FurLengthMask, input.texcoord).r / 10;
    output.furMask = furMask;
    output.id = input.id;
...
void geom(triangle Attributes input[3], inout TriangleStream<Varyings> stream)
{
    ...
    // Instead of sampling here, we use the interpolated value from the vertex shader.
    float furMask = input[0].furMask;
    float finalFurLength = _FurLength * furMask;

    float3 startCenterPosOS = (prevPos0OS + prevPos1OS + prevPos2OS) / 3;
    float3 topPosOS = startCenterPosOS + faceNormalOS * finalFurLength;
    ...
    float3 topMovedPosOS = startCenterPosOS + movedFaceNormalOS * finalFurLength;
